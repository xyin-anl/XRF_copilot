...
def simulate_XRF_maps(params):
    ...
    n_line_group_each_element = np.array(
        [np.sum(element_lines_roi[:, 0] == name) for name in elements]
    )
    if debug:
    ...
    detected_fl_unit_concentration = (
        tc.as_tensor(fl_all_lines_dic["detected_fl_unit_concentration"]).float().to(dev)
    )

    # Get the mass attenuation cross section for each XRF line (3rd row in Table 5.3.1) as a list ####
    mass_attenuation_cross_section_FL = (
        tc.as_tensor(
            xlib_np.CS_Total(
                np.array(list(atomic_numbers.values())), fl_all_lines_dic["fl_energy"]
            )
        )
        .float()
        .to(dev)
    ...
    probe_attCS_ls = tc.as_tensor(
        xlib_np.CS_Total(
            np.array(list(atomic_numbers.values())), np.array([probe_energy])
        ).flatten()
    ).to(dev)
    ...
    det_solid_angle_ratio = (np.pi * (det_size_cm / 2) ** 2) / (
    ...
    output_dir = os.path.join(
        os.path.dirname(ground_truth_file),
    ...
    if not os.path.isfile(f"{output_dir}/P_array.h5") or overwrite_P:
        if debug:
            print(
                f"Calculating the intersecting length array P. This will take quite some time..."
            )
        intersecting_length_fl_detectorlet(
            n_ranks,
            rank,
            ...
            output_dir,
            "P_array",
        )  # has to use CPU for this step
        if debug:
            print(f"Completed. P is saved at {output_dir}/P_array.h5")
    else:
        if debug:
            print(
                f"Loading an existing intersecting length array P from {output_dir}/P_array.h5"
            )
    ...
    if not isinstance(rotation_angles, tc.Tensor):
        rotation_str = "_rot" + "_".join(
            [f"{angle:.2f}".replace(".", "p") for angle in rotation_angles]
        )
    else:
        rotation_str = "_rot" + "_".join(
            [
                f"{angle:.2f}".replace(".", "p")
                for angle in rotation_angles.cpu().numpy()
            ]
        )
    ...
    with h5py.File(sim_XRF_file, "w") as d:
        ...
        data = grp.create_dataset(
            "data", shape=(n_lines, sample_height_n, sample_size_n), dtype="f4"
        )
    ...
    with h5py.File(sim_XRT_file, "w") as d:
        grp = d.create_group("exchange")
        data = grp.create_dataset(
            "data", shape=(4, sample_height_n, sample_size_n), dtype="f4"
        )
    ...
    if not tc.all(rotation_angles == 0):
        print(
            f"Rotating object with rotation angles (degrees): {rotation_angles.cpu().numpy()}"
        )
        ...
        if rotation_angles[0] != 0:
            X_rot = rotate_3d_inplane(
                X_rot, rotation_angles[0], dev, use_degrees=True, axis="x"
            )

        # Apply Y-axis rotation if needed
        if rotation_angles[1] != 0:
            X_rot = rotate_3d_inplane(
                X_rot, rotation_angles[1], dev, use_degrees=True, axis="y"
            )

        # Apply Z-axis rotation if needed
        if rotation_angles[2] != 0:
            X_rot = rotate_3d_inplane(
                X_rot, rotation_angles[2], dev, use_degrees=True, axis="z"
            )
    ...
    for i in range(n_element):
        tifffile.imwrite(
            f"{output_dir}/X_{elements[i]}_rot"
            + "_".join([f"{angle:.2f}".replace(".", "p") for angle in rotation_angles])
            + ".tiff",
            X[i].cpu().numpy(),
        )
    ...
    if model_self_absorption == True:
        ...
        lac = X.view(n_element, 1, 1, n_voxel) * mass_attenuation_cross_section_FL.view(
    ...
    for m in batch_iterator:
        ...
        if model_self_absorption == True:
            ...
            if (
                start_idx >= P_handle["P_array"].shape[2]
                or end_idx > P_handle["P_array"].shape[2]
            ):
                if debug:
                    print(f"WARNING: Index out of bounds!")
                raise ValueError(f"Batch {m+1}: Index out of bounds in P_array access")

            P_minibatch = tc.from_numpy(
                P_handle["P_array"][:, :, start_idx:end_idx]
            ).to(dev)
        ...
        try:
            ...
            if debug:
                ...
                if not tc.all(rotation_angles == 0):
                    print(
                        f"Using 3D rotation with angles: {rotation_angles.cpu().numpy()}"
                    )
                print(
                    f"lac shape: {lac.shape if isinstance(lac, tc.Tensor) else 'scalar 0'}"
                )
                print(f"X shape: {X.shape}")
                print(f"n_element: {n_element}")
                print(f"n_lines: {n_lines}")
                print(
                    f"mass_attenuation_cross_section_FL shape: {mass_attenuation_cross_section_FL.shape}"
                )
                print(
                    f"detected_fl_unit_concentration shape: {detected_fl_unit_concentration.shape}"
                )
                print(f"n_line_group_each_element: {n_line_group_each_element}")
            ...
            try:
                if model_self_absorption:
                    ...
                    model = PPM(
                        ...
                        sample_size_cm,
                        probe_energy,
                        incident_probe_intensity,
                        model_probe_attenuation,
                        probe_attCS_ls,
                        0,
        ...
        with h5py.File(sim_XRF_file, "r+") as d:
            d["exchange/data"][
                :,
                batch_size * p // sample_size_n : batch_size * (p + 1) // sample_size_n,
                :,
    ...
    total_time = (
        datetime.datetime.now() - start_time_total
    )  # Calculate time taken for the whole simulation
    if debug:
    ...
    for i in range(n_element):
        tifffile.imwrite(
            f"{output_dir}/sim_xrf_E{probe_energy}{suffix}_{elements[i]}.tif",
            xrf_data[i],
        )
